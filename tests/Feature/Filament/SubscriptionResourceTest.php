...
it('exposes Refund Unused Days action on the view page only', function (): void {
...
it('processes refund via Refund Unused Days action without ledger reversals', function (): void {
    $user = authenticateSubscriptionFilamentUser();
    ['subscription' => $subscription] = createFilamentSubscriptionScenario();

    Livewire::actingAs($user)
        ->test(ViewSubscription::class, ['record' => $subscription->getRouteKey()])
        ->callAction('refundUnusedDays', data: [
...
it('hides refund action after refund and keeps a single refund record', function (): void {
    ...
    $component->callAction('refundUnusedDays', data: [
        'cancellation_date' => '2026-01-10',
    ]);
